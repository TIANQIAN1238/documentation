...
### 在 Workflow {#handle-cancellation-in-Workflow} 中处理取消
...
### 在 Activity {#handle-cancellation-in-an-Activity} 中处理取消
确保 Activity 是 [心跳](/develop/dotnet/activities/timeouts#activity-heartbeats) 以接收
取消请求并停止执行。还要确保
调用时，[心跳超时](/develop/dotnet/activities/timeouts#heartbeat-timeout) 在 Activity 选项上设置
...
在 `WorkflowHandle` 上使用 `CancelAsync` 取消 Workflow Execution。
...
<a id="how-to-request-cancellation-of-an-activity"></a>
#### 如何请求取消 Activity {#how-to-request-cancellation-of-an-activity}

默认情况下，由于 Workflow 取消令牌而取消 Workflow 时，活动会自动取消
...
<a id="termination"></a>
## 终止 {#termination}
要终止 .NET 中的 Workflow Execution，请使用
[终止异步()](https://dotnet.temporal.io/api/temporalio.Client.WorkflowHandle.html#Temporalio_Client_WorkflowHandle_TerminateAsync_System_String_Temporalio_Client_WorkflowTerminateOptions_)
Workflow 句柄上的方法。
