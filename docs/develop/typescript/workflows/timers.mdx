...
tags:
  ...
  - TypeScript SDK
  - Temporal SDKs
description: "Workflow 使用 sleep() 或 Timer() 将持久的 Timer 设置为固定时间段。 Timer 被持久化，确保在停机后使用最少的资源继续执行。"
---
...
将 `Promise.race` 与 Timer 一起使用可动态调整延迟。
...
将 `Promise.race` 与 Signal 和触发器结合使用，可以在系统时间或人工干预中较早的时间进行承诺解决。
...
这可以在第三方包 [`Temporal-time-utils`](https://www.npmjs.com/package/temporal-time-utils#user-content-updatabletimer) 中找到，您还可以在其中看到实现：
