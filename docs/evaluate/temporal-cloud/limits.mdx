---
id: limits
title: "Temporal Cloud 系统限制"
sidebar_label: "限制"
slug: /cloud/limits
description: "了解 Temporal Cloud 的各项限制，包括账户、Namespace、吞吐量、保留期、任务轮询器、批处理作业、gRPC、Search Attribute 等。"
...
Temporal Cloud 通过多种限制来保障服务可靠性，包括速率限制（单位时间内可发生某操作的次数）、资源限制（某类资源可同时存在的数量）以及配置限制（某项设置的最小值或最大值）。

每一类限制都作用于特定范围（应用层级）：

- 在 Temporal Cloud [账户级别](#account-level)
- 在 [Namespace 级别](#namespace-level)
- 在 [Nexus 端点级别](#nexus-endpoint-level)
- 在 [编程模型层面](#programming-model-level)

## 账户级别 {#account-level}

以下限制适用于 Temporal Cloud 账户级别（每个账户）。
