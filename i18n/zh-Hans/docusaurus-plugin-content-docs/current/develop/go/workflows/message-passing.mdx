...
您将收到一个 [`ServiceError`](https://pkg.go.dev/go.Temporal.io/api/serviceerror#ServiceError)，其`status` 为 `FAILED_PRECONDITION`。

- **Query 失败。**
  您将收到 [`QueryFailed`](https://pkg.go.dev/go.Temporal.io/api/serviceerror#QueryFailed) 错误。
  Query 处理程序中的任何恐慌都会触发此错误。
  这与 Signal 和 Update 不同，后者的恐慌可能会导致 Workflow Task 故障。
