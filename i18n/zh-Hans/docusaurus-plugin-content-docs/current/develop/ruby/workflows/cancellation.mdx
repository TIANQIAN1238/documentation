...
## Workflow 中的取消处理 {#handle-cancellation-in-Workflow}
...
Workflow 也是如此（例如 Timer、活动等），因此取消会传播到它们进行处理，
气泡出来。
...
确保 Activity 处于 [Heartbeating](/develop/ruby/activities/timeouts#Activity-heartbeats) 状态以接收
取消请求并停止执行。还要确保
[Heartbeat Timeout](/develop/ruby/activities/timeouts#heartbeat-timeout) 在调用时在 Activity 选项上设置
...
使用“WorkflowHandle”上的“cancel”取消 Workflow Execution。
