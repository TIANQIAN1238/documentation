...
description: 本节介绍 Workflow 与 Ruby SDK 的未来
...
## Workflow 期货 {#Workflow-期货}

`Temporalio::Workflow::Future` 可用于在后台或同时运行。
在此类情况下，Temporal 围绕某些核心语言功能提供 Workflow 安全包装器。
“Temporalio::Workflow::Future”是“Fiber.Schedule”的安全包装，用于一次运行多个活动。
Ruby SDK 还提供“Workflow.wait_condition”用于等待结果。

期货从不隐式使用，但它们适用于所有 Workflow 代码和构造。
...
proc而不是块等。有关详细信息，请参阅[API文档](https://ruby.Temporal.io/Temporalio/Workflow/Future.html)。
