...
由于有效负载转换器在 Workflow 上下文中运行，因此它们在 Workflow 沙箱内运行。因此，有效负载转换器无法访问外部服务或使用非确定性模块，这由于其非确定性性质而排除了大多数类型的加密。
...
实际上，这两个组件在链中运行以安全地处理数据。传入数据首先通过“toPayload”方法通过有效负载转换器，将应用程序对象转换为有效负载。然后，这些 Payload 由 Payload Codec 通过“encode”方法进行处理，该方法根据所需的安全性或效率需求调整 Payload，然后再将其发送到 Temporal 集群。

该过程对于传出数据是对称的。从 Temporal 集群检索的有效负载首先通过“解码”方法通过有效负载编解码器，该方法会反转编码期间应用的任何转换。最后，有效负载转换器通过“fromPayload”方法将生成的有效负载转换回应用程序对象，使其可以在应用程序中使用。

## 负载编解码器

> API文档：[PayloadCodec](https://typescript.Temporal.io/api/interfaces/common.PayloadCodec)
...
Temporal SDK 提供了一个 [Payload Converter](/payload-converter)，可进行自定义以将自定义数据类型转换为 [Payload](/dataconversion#payload) 并返回。

编码有效负载转换器的应用顺序取决于为 Data Converter 指定的顺序。
您可以设置多个编码有效负载转换器来运行转换。
当 Data Converter 接收到要转换的值时，该值将按顺序传递通过每个有效负载转换器，直到处理该数据类型的转换器进行转换。

## 复合 Data Converters

使用 [Composite Data Converter](https://typescript.Temporal.io/api/classes/common.CompositePayloadConverter) 按指定顺序应用自定义、特定于类型的有效负载转换器。实现自定义数据处理并不总是需要定义新的 Composite Data Converter。您可以使用自定义编解码器覆盖默认转换器，但复杂的 Workflow 逻辑可能需要复合 Data Converter。

复合 Data Converter 可以包含创建的自定义规则，它还可以
利用 Temporal 内置的默认 Data Converter。事实上，默认
Data Converter 逻辑在 Temporal 源内部实现为
复合Data Converter。它按以下顺序定义这些规则：
...
在上面的代码片段中，创建了一个转换器，它首先尝试处理“null”和“undefined”值。如果该值不是“null”或“undefined”，则使用“EjsonPayloadConverter”中编写的EJSON序列化逻辑。然后将有效负载转换器提供给 Worker 和客户端。
...
在 XPROT022X 中分配资源的公平权重。例如，假设您有三个公平键来描述
...
要支持自定义有效负载转换，请创建一个[自定义有效负载转换器](/payload-converter#composite-data-converters) 并配置 Data Converter 以在客户端选项中使用它。
您可以使用自定义有效负载转换器来更改应用程序对象序列化为二进制有效负载的方式。要处理本机不可序列化的自定义数据类型（例如“BigInt”、“Date”或二进制数据），您可以创建自定义有效负载转换器。自定义有效负载转换器负责将您的自定义数据类型转换为 Temporal 可以管理的有效负载格式。
...
此自定义转换器应包含序列化（“toPayload”）和反序列化（“fromPayload”）的逻辑，处理您的特定数据类型或序列化格式。 toPayload 方法返回一个 Payload 对象，用于管理和传输序列化数据。 “fromPayload”方法返回反序列化的数据。这可以确保返回的数据与序列化之前的格式相同，从而可以直接在应用程序中使用。

2. 配置Data Converter。要发送不可 JSON 序列化的值（如“BigInt”或“Date”），请向客户端和 Worker 提供自定义 Data Converter，如 [Composite Data Converters](#composite-data-converters) 部分中所述。
...
示例项目 [samples-typescript/ejson](https://github.com/Temporalio/samples-typescript/tree/main/ejson) 创建一个 EJSON 自定义“PayloadConverter”。
它实现了 `PayloadConverterWithEncoding` 而不是 `PayloadConverter`，以便它可以与 [CompositePayloadConverter](https://typescript.Temporal.io/api/classes/common.CompositePayloadConverter/) 一起使用：
...
将值序列化为[协议缓冲区](https://Protobuf.dev/) (Protobufs)：
...
- 创建一个 [`DefaultPayloadConverterWithProtobufs`](https://typescript.Temporal.io/api/classes/Protobufs.DefaultPayloadConverterWithProtobufs/)：
...
如果我们知道我们只使用 Protobuf 对象，并且希望它们进行二进制编码（这比 proto3 JSON 节省空间，但无法在 Web UI 中查看），我们可以执行以下操作：
...
- 在 Workflow 和活动中使用 Protobuf：

<!--SNIPSTART 打字稿-Protobuf-Workflow -->
...
<!--SNIPSTART 打字稿-Protobuf-Activity -->
...
<!--SNIPSTART 打字稿加密编解码器 -->
[加密/src/加密-codec.ts](https://github.com/Temporalio/samples-typescript/blob/main/加密/src/加密-codec.ts)
```ts
...
加解密代码位于[src/crypto.ts](https://github.com/Temporalio/samples-typescript/tree/main/encryption/src/crypto.ts)。
由于加密是 CPU 密集型的，并且使用 Node.js 内置的加密模块执行 AES 会阻塞主线程，因此我们使用“@ronomon/crypto-async”，它使用 Node.js 线程池。

和以前一样，我们向客户端提供定制的 Data Converter 和 Worker：
...
当客户端向 Workflow 发送“Alice：Bob 的私人消息”时，它会在客户端上加密并在 Worker 中解密。
Workflow 接收解密的消息并附加另一条消息。
当它返回较长的字符串时，该字符串将由 Worker 加密并由客户端解密。

<!--SNIPSTART 打字稿加密-Workflow -->
