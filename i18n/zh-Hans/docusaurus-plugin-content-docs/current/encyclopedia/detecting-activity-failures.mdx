...
sidebar_label: 活动
description: 了解 Activity Temporal 中的执行超时； Schedule-To-Start、Start-To-Close、Schedule-To-Close 和 Activity 心跳，用于有效的 Workflow 管理。
...
Workflow 可以通过以下超时检测不同类型的 Activity 执行故障：

- [Schedule-启动超时](#Schedule-启动超时)
- [开始到关闭超时](#start-to-close-timeout)
- [Schedule-关闭超时](#Schedule-关闭超时)
- [Activity 心跳](#Activity-心跳)

## Schedule-启动超时 {#Schedule-启动超时}

**什么是 Temporal 中的 Schedule 启动超时？**

Schedule 启动超时是从 [Activity 任务](/tasks#Activity-task) 执行 Scheduled（即放置在 Task Queue 中）到 [Worker](/Workers#Worker) 启动（即从Task Queue）即 Activity 任务。
换句话说，这是 Activity 任务排队时间的限制。
